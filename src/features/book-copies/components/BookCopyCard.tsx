...
export function BookCopyCard({ copy, onEdit, onDelete, showActions = false, onClick, bookmark }: BookCopyCardProps) {
  return (
    <div
      onClick={onClick}
      className={cn(
        'bg-bg-light-2 rounded-2xl border border-border flex flex-col shadow-sm overflow-visible',
        onClick && 'cursor-pointer hover:shadow-lg hover:-translate-y-1 transition-all',
      )}
    >
      <div className="aspect-[2/3] relative flex-shrink-0 overflow-visible rounded-t-xl">
        {copy.cover_image_url ? (
          <Image src={copy.cover_image_url} alt={copy.title || ''} fill className="object-cover rounded-t-xl" />
        ) : (
